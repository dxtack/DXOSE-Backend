...
const MATRIX_PATH = path.join(ROOT, 'Governance/constitution-coverage/FULL_CONSTITUTION_COVERAGE_393_MATRIX_SEMANTIC_FINAL.json');
const V3_PATH = path.join(ROOT, 'Governance/runtime-revalidation/P0_RUNTIME_V3_FINAL.json');
const OUT_PATH = path.join(ROOT, 'Governance/constitution-coverage/SEMANTIC_EVIDENCE_INTEGRITY_VALIDATION.json');
...
function main() {
  ...
  try {
    const buildSrc = fs.readFileSync(path.join(ROOT, 'Governance/constitution-coverage/build-semantic-final-matrix.cjs'), 'utf8');
